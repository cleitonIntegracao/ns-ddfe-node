...
class Response {
    constructor({ status, ultNSU, xmls, nsu, chave, emitCnpj, emitRazao, cSitNFe, modelo, vNF, tpEvento, xml, pdf }) {
        this.status = status;
        this.ultNSU = ultNSU;
        this.xmls = xmls;
        this.nsu = nsu;
...
async function sendPostRequest(body, caminho) {
    ...
    if (responseAPI.pdf != null) {
        ...
        util.salvarArquivo(caminho, responseAPI.chave, "-nfeProc.pdf", buff)
    }

    if (responseAPI.xml != null) {
        util.salvarArquivo(caminho, responseAPI.chave, "-nfeProc.pdf", responseAPI.xml)
    }
